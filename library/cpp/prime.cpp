...
vector<int> create_is_prime(int n) {
    vector<int> is_prime(n+1, 1);
    is_prime[0] = is_prime[1] = 0;
    FOR(i, 2, n+1) {
        if (!is_prime[i]) continue;
        for (auto j = i*2; j <= n; j += i) is_prime[j] = false;
    }
    return is_prime;
}

bool is_prime_n(ll n) {
    auto sqrtn = sqrt(n);
    FOR(i, 2, sqrtn+1) {
...
vector<int> create_n_divisor(int n) {
    vector<int> n_divisor(n+1, 0);
    FOR(i, 1, n+1) {
...
void _main() {
    auto is_prime = create_is_prime((int)1e7);
    cout << is_prime[(int)1e7] << endl;

    cout << is_prime_n((ll)1e16) << endl;

    auto n_divisor = create_n_divisor(13);
    DEBUG(n_divisor);
}
