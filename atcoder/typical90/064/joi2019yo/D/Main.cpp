...
bool exist(const vector<int> &A, int i, int h) {
    if (i < 0 || A.size() <= i) return false;
...
void _main() {
    ...
    FOR(i, 1, N) {
        ...
        H[A[idx]].push_back({idx, i-1});
        idx = i;
    }
    H[A[idx]].push_back({idx, N-1});
    ...
    int current = ans;
    for (const auto &[h, ps] : H) {
        DEBUG(h, ps);
        if (h == 0) continue;
        for (const auto &p : ps) {
            auto l = p.first;
            auto r = p.second;
            if (!exist(A,l-1,h) && !exist(A,r+1,h)) --current;
            if (exist(A,l-1,h) && exist(A,r+1,h)) ++current;
        }
        ans = max(ans, current);
        DEBUG(current, ans);
