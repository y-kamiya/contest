	...
	long long findMin (int N, vector <int> s, vector <int> t, vector <int> weight, int charges) {
        ...
        FOR(k,charges+1) {
            while (true) {
                bool update = false;
                FOR(i,E) {
                    int start = s[i], end = t[i], w = weight[i];
                    if (cost[start][k] != INF && cost[end][k] > cost[start][k] + w) {
                        cost[end][k] = cost[start][k] + w;
                        update = true;
                    }
                    if (k != 0 && cost[end][k] > cost[start][k-1] - w) {
                        cost[end][k] = cost[start][k-1] - w;
                        update = true;
                    }
                }
                if (!update) break;
